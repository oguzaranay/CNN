...
class ValueNetwork(object):
    ...
    def train(self, train_features, train_labels, epochs=1000, batch_size=20, train_ratio=0.9):
        ...
        for epoch in range(0, epochs):
            ...
            while True:
                data = queue.get(timeout=10)  # batch size = 20, data dim (20,1836)
                if data is not self.sentinel:
                    # Do a training step to learn to correctly score the solution (predicted labels)
...
#                self.saver.save(self.sess, '%s/weights' % self.data_dir, global_step=self.current_step)
            ...
            if len(f1_scores) > 0:
                ...
                print('Validation mean IOU: %.3f' % np.mean(f1_scores[:, 1]))
                # fp.write(str(np.mean(f1_scores[:, 1])) + '\n')
                sys.stdout.flush()
    ...
    def generate_examples(self, features, gt_labels, train=False, val=False):
        ...
        if train and np.random.rand() >= 0.5:
            ...
            pred_labels = self.inference(features, init_labels, gt_labels=gt_labels, num_iterations=1,
                                         learning_rate=self.inf_lr, )# case of using adversarial examples
            # pred_labels = gt_labels # case of using ground truths
            log = False
        ...
        # labels[:, 1] = [self.gt_value(pred_labels[idx], gt_labels[idx], train=train) for idx in # labels[:, 1] for gt_f1_score
        #                 np.arange(0, gt_labels.shape[0])]
        labels[:, 1] = [self.gt_value_iou(pred_labels[idx], gt_labels[idx], train=train) for idx in # labels[:, 1] for iou score
                        np.arange(0, gt_labels.shape[0])]
        labels[:, 0] = 1 - labels[:, 1]                                                             # labels[:, 0] for the loss
    ...
    def gt_value_iou(self, pred_labels, gt_labels, train=True):
        """Compute the IOU performance of a predictor on the given data.""" # IOU
#        mean_iou = []
...
#        fp.close()
    ...
    def gt_value(self, pred_labels, gt_labels, train=True):
        """Compute the ground truth value of some predicted labels.""" # f1 score
    ...
    def inference(self, features, initial_labels, gt_labels=None, learning_rate=0, num_iterations=20):
        ...
        for idx in range(0, num_iterations):
            ...
            if gt_labels is not None:  # Adversarial
                ...
                #                    np.arange(0, gt_labels.shape[0])] # with f1 score metric
                gt_values[:, 1] = [self.gt_value_iou(pred_labels[idx], gt_labels[idx]) for idx in
                                   np.arange(0, gt_labels.shape[0])] # with IOU metric
                gt_values[:, 0] = 1 - gt_values[:, 1]

                if idx == 0 and self.writer:
                    gradient, summary_str = self.sess.run([self.adv_gradient, self.inf_summary_op],
                                                          feed_dict={self.features_pl: features,
                                                                     self.labels_pl: pred_labels,
                                                                     self.gt_scores_pl: gt_values})
                else:
                    gradient = self.sess.run(self.adv_gradient,
                                             feed_dict={self.features_pl: features,
                                                        self.labels_pl: pred_labels,
                                                        self.gt_scores_pl: gt_values})
            else:
            ...
            pred_labels += learning_rate * gradient     # eq. 8

            # Project back to the valid range
            # pred_labels[np.isnan(pred_labels)] = 0
            pred_labels[pred_labels < 0] = 0            # eq. 8
            pred_labels[pred_labels > 1] = 1            # eq. 8
    ...
    def get_prediction(self, feature_input, label_input):
        ...
        W = tf.Variable(tf.random_normal([self.feature_dim, self.num_hidden],
                                         stddev=np.sqrt(2.0 / (self.feature_dim))))  # weights: 1836 x 150
        b = tf.Variable(tf.zeros([self.num_hidden]))  # biases: 150 x 1

        # Add L_x regularization
        self.loss += self.weight_decay * self.regularizer(W)  # compute half of L2 norm w/o sqrt

        prediction1 = self.nonlinearity(tf.matmul(feature_input, W) + b)  # compute softplus: log(exp(input) + 1)
        ...
        prediction1 = tf.matmul(prediction1, W2) + b2  # size: n x 159
    ...
    def build_graph(self):
        ...
        summaries = [tf.summary.scalar('loss', tf.reduce_mean(self.loss)),
                     tf.summary.histogram('predicted_iou_scores', predicted_values)]

        # self.gt_dist_op = tf.summary.scalar('gt_f1_scores', tf.reduce_mean(self.gt_scores_pl[:, 1]))
        self.gt_dist_op = tf.summary.scalar('gt_iou_scores', tf.reduce_mean(self.gt_scores_pl[:, 1]))
        ...
        def generate():
            try:
                while True:
                    ...
                    _queue.put((features, pred_labels, f1_scores))
            except queue.Empty:
                _queue.put(self.sentinel)
